...
function bp_legacy_theme_post_update() {
	...
	if ( empty( $_POST['content'] ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error bp-ajax-message alert alert-danger"><p>' . __( 'Please enter some content to post.', 'firmasite' ) . '</p></div>' );
	...
	if ( empty( $activity_id ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error bp-ajax-message alert alert-danger"><p>' . __( 'There was a problem posting your update. Please try again.', 'firmasite' ) . '</p></div>' );
...
function bp_legacy_theme_new_activity_comment() {
	...
	if ( empty( $_POST['content'] ) ) {
		exit( '-1<div class="clearfix"></div><div id="message" class="error bp-ajax-message alert alert-danger"><p>' . esc_html__( 'Please do not leave the comment area blank.', 'firmasite' ) . '</p></div>' );
	}
	
	if ( empty( $_POST['form_id'] ) || empty( $_POST['comment_id'] ) || ! is_numeric( $_POST['form_id'] ) || ! is_numeric( $_POST['comment_id'] ) ) {
		exit( '-1<div class="clearfix"></div><div id="message" class="error bp-ajax-message alert alert-danger"><p>' . esc_html( $feedback ) . '</p></div>' );
	}
...
function bp_legacy_theme_delete_activity() {
	...
	if ( ! bp_activity_delete( array( 'id' => $activity->id, 'user_id' => $activity->user_id ) ) )
		exit( '-1<div class="clearfix"></div><div id="message" class="error bp-ajax-message alert alert-danger"><p>' . __( 'There was a problem when deleting. Please try again.', 'firmasite' ) . '</p></div>' );
